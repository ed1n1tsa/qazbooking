...
class QazbookingSupabaseUser extends BaseAuthUser {
  QazbookingSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> qazbookingSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = QazbookingSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
